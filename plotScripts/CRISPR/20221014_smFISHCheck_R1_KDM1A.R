...
dataDirectory <- "/Users/naveenjain/Dropbox (RajLab)/Shared_Naveen/Original Manuscript/rawData/CRISPR/R1_smFISH/UBC A594, KDM1A CY3/"
...
spotTablePlot <- spotTable %>% dplyr::filter(channel == "tmr")
...
ggplot(spotTablePlot, aes(x = label, y = GroupCount)) +
  ...
  geom_signif(comparisons = list(c("control", "LSD1_1"), c("control", "LSD1_2"), c("control", "LSD1_3"), c("control", "LSD1_4")), step_increase = 0.075, map_signif_level = TRUE) +
...
ggsave(filename = paste0(plotDirectory, "smFISHCheckGraphs_violinOnly_R1_LSD1.pdf"), plot = plot1, units = "in", height = 3, width = 4.5, useDingbats = FALSE)
...
ggsave(filename = paste0(plotDirectory, "smFISHCheckGraphs_barOnly_R1_LSD1.pdf"), plot = plot2, units = "in", height = 3, width = 4.5, useDingbats = FALSE)

plot <- egg::ggarrange(plots = list(plot2, plot1), ncol = 2)
ggsave(filename = paste0(plotDirectory, "smFISHCheckGraphs_R1_LSD1.pdf"), plot = plot, units = "in", height = 3, width = 6, useDingbats = FALSE)
