...
dataDirectory <- "/Users/naveenjain/Dropbox (RajLab)/Shared_Naveen/Paper/rawData/CRISPR/R1_smFISH/UBC A594, KDM1A CY3/"
...
plot1 <- ggplot(spotTablePlot, aes(x = label, y = GroupCount, fill = label)) +
  ...
  geom_signif(comparisons = list(c("control", "LSD1_1"), c("control", "LSD1_2"), c("control", "LSD1_3"), c("control", "LSD1_4")), step_increase = 0.125) +
  theme(axis.title.x = element_blank()) + ylab("RNA counts per cell") +
