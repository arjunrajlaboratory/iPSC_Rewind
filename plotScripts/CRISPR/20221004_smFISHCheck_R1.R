...
plot2 <- ggplot(spotTablePlot, aes(x = label, y = GroupCount, fill = label)) +
  ...
  scale_fill_manual(values = c(rep("#D8D8D8", 1), rep("#ED1C24", 4)))
ggsave(filename = paste0(plotDirectory, "smFISHCheckGraphs_barOnly_R1.pdf"), plot = plot2, units = "in", height = 3, width = 4.5, useDingbats = FALSE)

plot <- egg::ggarrange(plots = list(plot2, plot1), ncol = 2)
ggsave(filename = paste0(plotDirectory, "smFISHCheckGraphs_R1.pdf"), plot = plot, units = "in", height = 3, width = 6, useDingbats = FALSE)
