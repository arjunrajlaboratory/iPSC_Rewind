...
spotTableCast$genesCorr <- spotTableCast$genes
...
ggplot(spotTableCast %>% filter(genesCorr != "KDM1A"), aes(x = genesCorr, y = prolifPerDay)) +
  ...
  NoLegend() + theme(axis.title.x = element_blank()) + ylab("proliferation rate per day") + ylim(0, 3)
ggsave(filename = paste0(plotDirectory, "prolifRatePerCRISPRGuideAggregate.pdf"), units = "in", height = 3, width = 4, useDingbats = FALSE)
