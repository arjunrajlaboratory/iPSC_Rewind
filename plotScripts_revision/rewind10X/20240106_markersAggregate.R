...
homeDirectory <- "/Users/naveenjain/Dropbox (RajLab)/Shared_Naveen/Original Manuscript/extractedData/rewind10X/"
plotDirectory <- "/Users/naveenjain/Dropbox (RajLab)/Shared_Naveen/Revised Manuscript/plots/rewind10X/"

markers_R1 <- readRDS("/Users/naveenjain/Dropbox (RajLab)/Shared_Naveen/Original Manuscript/extractedData/rewind10X/R1/primedMarkersAll.rds") %>% mutate(rep = "1")
markers_R2 <- readRDS("/Users/naveenjain/Dropbox (RajLab)/Shared_Naveen/Original Manuscript/extractedData/rewind10X/R2/primedIndMarkersAll.rds") %>% mutate(rep = "2")
markers_R3 <- readRDS("/Users/naveenjain/Dropbox (RajLab)/Shared_Naveen/Original Manuscript/extractedData/rewind10X/R3/primedMarkersAll.rds") %>% mutate(rep = "3")
...
library(Seurat)
library(ggrepel)
ccGeneList <- c(cc.genes.updated.2019$s.genes, cc.genes.updated.2019$g2m.genes)
markersCC <- markersComb %>% dplyr::filter(gene %in% ccGeneList)

ggplot(markersCC, aes(x = 1, y = mean)) +
  geom_point(position = position_jitter(seed = 1234, height = 0)) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_text_repel(aes(label = gene), position = position_jitter(seed = 1234, height = 0), size = 2.5) +
  ylim(-0.5, 0.5) +
  theme(axis.title.x = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank())
ggsave(filename = paste0(plotDirectory, "cellCycleMarkersLog2FC.pdf"), height = 4, width = 2.5)
