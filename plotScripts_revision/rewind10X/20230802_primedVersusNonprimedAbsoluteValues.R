...
homeDirectory <- "/Users/naveenjain/Dropbox (RajLab)/Shared_Naveen/Original Manuscript/extractedData/rewind10X/R1/"
...
barcodesR1 <- read.table("/Users/naveenjain/Dropbox (RajLab)/Shared_Naveen/Original Manuscript/extractedData/rewind10X/R1/stepThreeStarcodeShavedReads_BC_10XAndGDNA.txt", header = TRUE, sep = '\t')
...
ggplot(logNormPlotSelectMelt, aes(x = label, y = value)) + 
  ...
  facet_wrap(~variable, nrow = 2) + NoLegend() + ylab("log(normalized RNA counts)") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1), axis.title.x = element_blank())
ggsave(filename = paste0(plotDirectory, "primingMarkersAbsoluteValues_boxplots.pdf"), height = 4, width = 6)
