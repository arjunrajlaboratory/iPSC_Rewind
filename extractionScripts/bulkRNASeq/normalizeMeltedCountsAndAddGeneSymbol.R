...
meltedDataInputFile   <- '/Users/naveenjain/Dropbox (RajLab)/Shared_Naveen/Paper/extractedData/bulkRNASeq/20220914_RNASeq_ProlifSort_Boosters_meltedData.tsv'
gtfFileUsedByPipeline <- '/Users/naveenjain/Dropbox (RajLab)/Shared_Naveen/Paper/extractionScripts/bulkRNASeq/hg38/hg38.gtf'
gtfFileDirectory      <- '/Users/naveenjain/Dropbox (RajLab)/Shared_Naveen/Paper/extractionScripts/bulkRNASeq/hg38/'
ENSGtoGeneSymbolTable <- '/Users/naveenjain/Dropbox (RajLab)/Shared_Naveen/Paper/extractionScripts/bulkRNASeq/geneSymbolConversionTables/hg38_EnsgHgncSymbolMapping.tsv'
outputFile            <- '/Users/naveenjain/Dropbox (RajLab)/Shared_Naveen/Paper/extractedData/bulkRNASeq/20220914_RNASeq_ProlifSort_Boosters_finalData.tsv'
...
htseq.table.withLength <- dplyr::inner_join(htseq.table, lengthtbl, by = 'gene_id') # this step also removes non_gene features from the table, e.g. "__no_feature"
htseq.table.withRPM <- htseq.table.withLength %>% 
  dplyr::group_by(experiment, sampleID) %>%
  dplyr::mutate(totalMappedReads = sum(counts), rpm = 1000000*counts / totalMappedReads)
htseq.table.withRPKM <- htseq.table.withRPM %>%
  dplyr::mutate(rpkm = 1000*rpm/length)
...
htseq.table.withTPM <- htseq.table.withRPKM %>%
  dplyr::mutate(ctsOverLength = counts/length, denominator = sum(ctsOverLength), tpm = ctsOverLength/denominator * 1000000) %>%
...
HGNC.symbol.table <- HGNC.symbol.table %>% dplyr::mutate(gene_id = ensg) %>% dplyr::select(-ensg)
