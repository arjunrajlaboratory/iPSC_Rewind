...
dataDirectory <- "/Users/naveenjain/Dropbox (RajLab)/Shared_Naveen/Original Manuscript/rawData/CRISPR/R2_smFISH/UBC A594, SPP1 CY5/"
plotDirectory <- "/Users/naveenjain/Dropbox (RajLab)/Shared_Naveen/Original Manuscript/plots/CRISPR/R2/"

folders <- list.dirs(path = dataDirectory, full.names = FALSE)[-1]

for(i in 1:length(folders)){
  if(i == 1) {
    spotTable <- read_csv(file = paste0(dataDirectory, folders[i], "/spotsSummary.csv")) %>% mutate(condition = rep(folders[i], nrow(.)))
  }
  spotTableTemp <- read_csv(file = paste0(dataDirectory, folders[i], "/spotsSummary.csv")) %>% mutate(condition = rep(folders[i], nrow(.)))
...
spotTablePlot <- spotTable %>% dplyr::filter(channel == "cy") %>% dplyr::filter(!(condition %in% c("C2", "C4", "B1_1", "B1_2")))
...
ggplot(spotTablePlot, aes(x = label, y = GroupCount)) +
  ...
  geom_signif(comparisons = list(c("control", "SPP1_1"), c("control", "SPP1_2"), c("control", "SPP1_3"), c("control", "SPP1_4")), step_increase = 0.075, map_signif_level = TRUE) +
...
ggsave(filename = paste0(plotDirectory, "smFISHCheckGraphs_violinOnly_R2_SPP1.pdf"), plot = plot1, units = "in", height = 3, width = 4.5, useDingbats = FALSE)
...
ggsave(filename = paste0(plotDirectory, "smFISHCheckGraphs_barOnly_R2.pdf"), plot = plot2, units = "in", height = 3, width = 4.5, useDingbats = FALSE)

plot <- egg::ggarrange(plots = list(plot2, plot1), ncol = 2)
ggsave(filename = paste0(plotDirectory, "smFISHCheckGraphs_R2.pdf"), plot = plot, units = "in", height = 3, width = 6, useDingbats = FALSE)
